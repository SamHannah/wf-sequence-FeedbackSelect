# runs a simulation of a word frequency sequence effect experiment accuracy feedback across all trials, but only for
# either correct or incorrect responses, at some level of L, F, SF and dp1,
...
#       (and SEs) for each list (high/low positive feedback levels); 
# 2) the quartile means (and SEs) for all four test item types (LF lures, etc.) for each list;
# 3) the four overall means and SEs for each list; 
...
# 5) the distribution of intensities  across test items and lists.
sequence<- function(SF1, dp1, L, A, N_subjects, fback){
  library(dplyr) 
  HF = 5; LF = 1
  Frequency<- 0    
  N_el = 100; N_items = 40
  ...
  for (N_reps in 1: N_subjects) {
    ...
    for (j in 1:N_test) {
      ...
      #selective change to dp based on feedback strategy (inform on errors, correct or none) and response
      if (fback == "Errors" && SF != 0) {                                 # feedback on errors (below criterion & item old, or above & item new)
        if ((Evidence < 0.0 && (testOrder[j] == 1 || testOrder[j] == 2)) || 
              (Evidence > 0.0 && (testOrder[j] == 3 || testOrder[j] == 4))){
          dp<- dp1/SF
        }else{
          dp<- dp1
        }
      }else if (fback == "Correct"){                             # feedback on correct (below criterion & item new, or above & item old)
        if ((Evidence > 0.0 && (testOrder[j] == 1 || testOrder[j] == 2)) || 
              (Evidence < 0.0 && (testOrder[j] == 3 || testOrder[j] == 4))){
          dp<- dp1*SF
        }else{
  ...
  dPold<- data.frame(Half=factor(N_seqSegment*2, levels = seq(1:N_seqSegment)), "Test.Item"=factor(N_seqSegment*2, 
                    levels=c("Lure", "Target")), "Mean.deltaP" = numeric(N_seqSegment*2),SE = numeric(N_seqSegment*2))
  dPold[,1]<- rep(seq(1:N_seqSegment), times=2)
  dPold[1:N_seqSegment,2]<- "Lure"
  dPold[(N_seqSegment+1):(2*N_seqSegment),2]<- "Target"
  dPold[1:N_seqSegment,3]<- dpOldMeans[,1]
  dPold[(N_seqSegment+1):(2*N_seqSegment),3]<- dpOldMeans[,2]
  dPold[1:N_seqSegment,4]<- dpOldError[,1]
  dPold[(N_seqSegment+1):(2*N_seqSegment),4]<- dpOldError[,2] 
  ...
  qMeans[,1]<- c(rep(seq(1:N_seqSegment), times=N_types))
  qMeans[1:N_seqSegment,2]<- "HFnew(LFnew)"
  qMeans[N_seqSegment+1:(2*N_seqSegment),2]<- "HFnew(LFold)"
  qMeans[(2*N_seqSegment+1):(3*N_seqSegment),2]<- "HFold(LFnew)"
  qMeans[(3*N_seqSegment+1):(4*N_seqSegment),2]<- "HFold(LFold)"
  qMeans[1:N_seqSegment,3]<- quartMeans[,1]
  qMeans[N_seqSegment+1:(2*N_seqSegment),3]<- quartMeans[,2]
  qMeans[(2*N_seqSegment+1):(3*N_seqSegment),3]<- quartMeans[,3]
  qMeans[(3*N_seqSegment+1):(4*N_seqSegment),3]<- quartMeans[,4]
  qMeans[1:N_seqSegment,4]<- quartError[,1]
  qMeans[N_seqSegment+1:(2*N_seqSegment),4]<- quartError[,2]
  qMeans[(2*N_seqSegment+1):(3*N_seqSegment),4]<- quartError[,3]
  qMeans[(3*N_seqSegment+1):(4*N_seqSegment),4]<- quartError[,4]
