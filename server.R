...
shinyServer(
  function(input, output) {
    seqOut<- reactive({
      sequence(input$SF, as.numeric(input$dp1),input$L, input$A, as.numeric(input$N_subjects), input$Feed)
      })  # returns an object named "outs", a list of 5 data frames
    
    output$all<- renderPlot({
        if (input$runSim == 0) return()
        warning("Simulation underway. Please stand back.")
    ...
    plotOutput<- function(outs){
      ...
      dp<- ggplot(outs[[1]], aes(Half, Mean.deltaP, group= Test.Item, linetype=Test.Item, shape=Test.Item))+
                        ...
                        labs(x="Trial Half", y=expression("Mean "~Delta~italic(P)~"('old')"))+
                        themePro
      ...
      means<- ggplot(outs[[3]], aes(Test.Item, Mean.P.old))+geom_bar(stat="identity", fill=c("grey55", "black","black","grey55"))+
                        ...
                        labs(x="Test Items", y="Mean P('old')")+
                        themePro+theme(legend.position="none")
      ...
      dists<- ggplot(outs[[5]], aes(bins, Count, group=Test.Item,linetype=Test.Item, 
                        ...
                        scale_colour_manual(name="Test item", values=c("blue","red","red", "blue"))+xlim(0.225,1.05)+
                        themePro
