shinyUI(fluidPage(
  titlePanel("Word frequency sequence effect simulator--Accuracy feedback & pairs"),
      ...
      fluidRow(
          column(6,
             helpText("This app runs a simulation of a word frequency sequence experiment (structured LF-HF pairs) using Minerva-AL and the criterion calibration rule for
              ...
              decrement parameter (dp) controlling the rate at which this shift factor decreases across trials is set by the decrement parameter (dp).Feedback
              changes the size of dp, with  positive feedback increasing it, and negative feedback decreasing it."),
          ...
          fluidRow(
            ...
            column(6,
                   sliderInput("SF", "Initial shift factor (SF)", min = 0.0, max=20.0, step=0.25, value=10.0)
            ),    
            column(6,
                   selectInput("dp1", "Initial decrement parameter (dp)",choices=c(0.0, 0.01, 0.025, 0.05, 0.1, 0.5, 1.25, 2.0), selected = 0.01)
            )
        ),
        fluidRow(
          column(4,
          ...
          column(4,
                 selectInput("Feed", "Responses to provide feedback for: ", choices=c("Errors", "Correct", "None"), selected = "None" )
            ),
